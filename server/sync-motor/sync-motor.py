...
class P2PClient:
    def __init__(
        ...
        if encryption_key:
            ...
            print(
                f"[{self.device_id}] \U0001f512 Clave de cifrado de contraseñas derivada con Argon2id"
            )
        else:
        ...
        self.multicast_group = (
            "239.255.0.42"  # Grupo multicast privado (rango 239.0.0.0/8 = admin-scoped)
        )
        self.multicast_port = 6003  # Puerto multicast para anuncios
        self.local_ip = (
            self._get_local_ip()
        )  # IP LAN para forzar multicast en la interfaz correcta
        ...
        self.multicast_listener_thread = threading.Thread(
            target=self.listen_multicast_announcements
        )
        ...
        self.multicast_broadcast_thread = threading.Thread(
            target=self.broadcast_announcements_via_multicast
        )
    ...
    def send_announcement_via_multicast(self):
        ...
        try:
            ...
            sock.close()
            print(
                f"[{self.device_id}] 📡 Anuncio enviado al grupo multicast {self.multicast_group}:{self.multicast_port}"
            )
            return True
        except Exception as e:
            print(
                f"[{self.device_id}] ⚠️  Error enviando anuncio multicast: {type(e).__name__}: {e}"
            )
            return False
    ...
    def broadcast_announcements_via_multicast(self):
        ...
        """
        print(
            f"[{self.device_id}] 📢 Iniciando broadcast periódico de anuncios vía MULTICAST (cada 20s)"
        )
        while self.running:
            try:
    ...
    def listen_multicast_announcements(self):
        ...
        try:
            ...
            print(
                f"[{self.device_id}] 📻 Escuchando anuncios en grupo multicast {self.multicast_group}:{self.multicast_port}"
            )
            ...
            while self.running:
                try:
                    ...
                    print(
                        f"[{self.device_id}] 📨 Anuncio recibido de {announcement['device_id']} desde {addr[0]}:{addr[1]}"
                    )
                    # Procesar el anuncio
                    device_id = announcement["device_id"]
                    peer_ip = addr[0]
                    peer_port = announcement.get(
                        "listen_port", self.listen_port
                    )  # Puerto TCP real, NO el UDP efímero

                    print(
                        f"[{self.device_id}] 📨 Anuncio de {device_id} con IP {peer_ip} y puerto TCP {peer_port} recibido"
                    )
                    ...
                    print(
                        f"[{self.device_id}] 📨 Anuncio de {device_id} con IP {peer_ip} y clave pública recibida"
                    )
                    ...
                    print(
                        f"[{self.device_id}] 📨 Anuncio multicast recibido de {device_id} desde {peer_ip}:{peer_port} - Clave pública y IP actualizadas ✅"
                    )

                except Exception as e:
                    if self.running:
                        print(
                            f"[{self.device_id}] ⚠️  Error procesando anuncio multicast: {type(e).__name__}"
                        )
        except Exception as e:
    ...
    def print_active_peers(self):
        ...
        for peer in peers:
            print(
                f"  • {peer['device_id']:<15} | IP: {peer['ip']:<15} | Puerto: {peer['port']:<5} | Visto hace {peer['last_seen_ago_seconds']}s"
            )
        print()

    def send_announcement_to_peer(
        self, peer_ip, peer_announcement_port, peer_device_id="unknown"
    ):
        """
        Envía un anuncio a demanda al grupo multicast
        """
    ...
    def add_new_password(
        ...
        encrypted_pwd = (
            password if _already_encrypted else self._encrypt_password(password)
        )
    ...
    def delete_password(self, site: str, user: str):
        ...
        now = int(time.time())
        self.vault.update_entry(site, user, {"state": "borrado", "timestamp_mod": now})
        ...
        return [
            {
                "domain": e.site,
                "user": e.user,
                "password": self._decrypt_password(e.password),
            }
    ...
    def save_password(self, domain: str, user: str, password: str):
        ...
        if existing:
            ...
            self.vault.update_entry(
                domain,
                user,
                {"password": encrypted_pwd, "state": "activo", "timestamp_mod": now},
            )
            self.vault.version += 1
            self.vault.timestamp = now
            self.vault._sync_metadata()
            ...
            return self.add_new_password(
                domain, user, password=encrypted_pwd, _already_encrypted=True
            )
    ...
    def establish_secure_channel(self, peer_public_key_bytes):
        ...
        peer_ephemeral_pub = CryptoUtils.deserialize_dh_public_key(
            peer_public_key_bytes
        )
        shared_secret = CryptoUtils.dh_shared_secret(
            my_ephemeral_priv, peer_ephemeral_pub
        )
        ...
        for entry in peer_vault_data["entries"]:
            match = next(
                (
                    ...
                    if e.site == entry["site"] and e.user == entry["user"]
                ),
                None,
            )
            if match:
                ...
                new_entry = VaultEntry(
                    entry["site"], entry["user"], entry["password"], entry["state"]
                )
    ...
    def listen_peer(self):
        ...
        while True:
            ...
            threading.Thread(
                target=self.handle_peer_connection, args=(client_sock, addr)
            ).start()
    ...
    def handle_peer_connection(self, client_sock, addr):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"[{self.device_id}] ⚠️ Datos no válidos recibidos de {addr}, ignorando"
                )
                client_sock.close()
                return
            ...
            shared_secret, my_ephemeral_pub_bytes = self.establish_secure_channel(
                peer_ephemeral_pub_bytes
            )
            print(f"[{self.device_id}] 🔐 Canal seguro X25519 establecido (servidor)")
            print(
                f"[{self.device_id}] SharedSecret (hex): {shared_secret.hex()[:16]}..."
            )
            ...
            print(
                f"[{self.device_id}] 🔑 SessionKey derivada: HKDF(SS, Token||Version||Timestamp)"
            )
            ...
            print(
                f"[{self.device_id}] 📥 Recibidos {len(data_metadata_enc)} bytes de metadata"
            )
            data_metadata_enc_dict = json.loads(data_metadata_enc.decode())
            metadata_json = CryptoUtils.aes_gcm_decrypt(
                session_key, data_metadata_enc_dict
            )
            ...
            if (
                peer_metadata["hash"] == my_metadata["hash"]
                and peer_metadata["version"] == my_metadata["version"]
            ):
                print(
                    f"[{self.device_id}] ✅ Vaults sincronizados (hash = version igual)"
                )
            ...
            if my_metadata["timestamp"] > peer_metadata["timestamp"]:
                ...
                print(
                    f"[{self.device_id}] 📥 Mi timestamp es mayor → Recibiendo vault de {peer_metadata['device_id']}"
                )
                response_sync = {"status": "merge_needed", "action": "send_vault"}
                # Cifrar respuesta con SessionKey
                response_sync_enc = CryptoUtils.aes_gcm_encrypt(
                    session_key, json.dumps(response_sync)
                )
                ...
                vault_json = CryptoUtils.aes_gcm_decrypt(
                    session_key, data_vault_enc_dict
                )
                response_vault = json.loads(vault_json)
                peer_vault_enc = response_vault[
                    "vault_encrypted"
                ]  # Dict con {nonce, ciphertext} - cifrado con master_password
                ...
                response_merged_enc = CryptoUtils.aes_gcm_encrypt(
                    session_key, json.dumps(response_merged, default=str)
                )
                client_sock.send(json.dumps(response_merged_enc).encode())
                print(
                    f"[{self.device_id}] ✅ Merge completado y vault enviado de vuelta"
                )
            else:
                # Mi timestamp es menor → Le envío mi vault EN LA MISMA RESPUESTA
                print(
                    f"[{self.device_id}] 📤 Mi timestamp es menor → Enviando vault a {peer_metadata['device_id']}"
                )
                ...
                merged_json = CryptoUtils.aes_gcm_decrypt(
                    session_key, data_merged_enc_dict
                )
    ...
    def synchronize_with_peer(self, peer_ip, peer_port):
        ...
        try:
            ...
            my_ephemeral_priv, my_ephemeral_pub = (
                CryptoUtils.generate_ephemeral_dh_keys()
            )
            my_ephemeral_pub_bytes = CryptoUtils.serialize_dh_public_key(
                my_ephemeral_pub
            )
            ...
            info = (
                auth_token["token"]
                + str(my_metadata["version"])
                + str(my_metadata["timestamp"])
            ).encode()
            session_key = CryptoUtils.hkdf(shared_secret, info)
            print(
                f"[{self.device_id}] 🔑 SessionKey derivada: HKDF(SS, Token||Version||Timestamp)"
            ...
            metadata_enc = CryptoUtils.aes_gcm_encrypt(
                session_key, json.dumps(my_metadata, default=str)
            )
            sock.send(json.dumps(metadata_enc).encode())
            print(
                f"[{self.device_id}] 📤 Enviada metadata CIFRADA (version: {my_metadata['version']})"
            )
            ...
            if sync_response["action"] == "send_vault":
                ...
                print(
                    f"[{self.device_id}] 📤 Peer tiene timestamp mayor → Enviando vault..."
                )
                my_vault_enc = self.encrypt_vault()
                ...
                response_with_vault_enc = CryptoUtils.aes_gcm_encrypt(
                    session_key, json.dumps(response_with_vault)
                )
                ...
                print(
                    f"[{self.device_id}] ✅ Sincronización completada (yo envié, peer hizo merge)"
                )

            elif sync_response["action"] == "receive_vault":
                # El peer tiene timestamp menor → Recibiré su vault en esta misma respuesta
                print(
                    f"[{self.device_id}] 📥 Peer tiene timestamp menor → Recibiendo vault..."
                )
                ...
                print(
                    f"[{self.device_id}] ✅ Sincronización completada (yo recibí y hacé merge)"
                )

        except Exception as e:
            print(
                f"[{self.device_id}] ❌ Error sincronización: {type(e).__name__}: {e}"
            )
            import traceback
...
if __name__ == "__main__":
    ...
    device_a = P2PClient(
        "DeviceA", master_password, listen_port=5001, announcement_port=6001
    )
    device_b = P2PClient(
        "DeviceB", master_password, listen_port=5002, announcement_port=6002
    )
    ...
    if devicea_sites == deviceb_sites and len(devicea_sites) == 2:
        ...
        for site in sorted(devicea_sites):
            print(f"   • {site}")
        print(
            f"   Hashes coinciden: {metadata_a_final['hash'] == metadata_b_final['hash']}"
        )
        print(
            f"   Versiones iguales: {metadata_a_final['version'] == metadata_b_final['version']}"
        )
    else:
    ...
    if devicea_entries == deviceb_entries:
        ...
        for site, state in sorted(devicea_entries):
            print(f"   • {site}: {state}")
        print(
            f"   Hashes coinciden: {metadata_a2_final['hash'] == metadata_b2_final['hash']}"
        )
        print(
            f"   Versiones iguales: {metadata_a2_final['version'] == metadata_b2_final['version']}"
        )
